...
\begin{frame}
  ...
  \item PRED：前驱元函数
    ...
    \end{displaymath} 
    \begin{itemize}
    \item 根据数学归纳法可以证明当$n > 0$的情况下$n \; (\lambda g \centerdot \lambda k \centerdot \; ISZERO \; (g \; 1) \; k \; (PLUS \; (g \; k) \; 1)) \; (\lambda v \centerdot \; 0)$就是加$n-1$次邱奇数1的函数
    \end{itemize} 
...
\subsection{\CJKfamily{hei}$\lambda$计算模型} 
\begin{frame}
  \frametitle{\CJKfamily{hei}$\lambda$计算模型：意义} 
  \begin{itemize}
...
\begin{frame}
  \frametitle{\CJKfamily{hei}递归实现：Y不动子}
\end{frame}
